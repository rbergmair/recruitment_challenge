...
def step17( datadir ):
  ...
  print( len(data_pos), N );
  
  #ratio = float( len(data_pos) ) / float( N )
  ratio = 0.162;
  
  pthresholds \
    = [ ratio * ( float(ii) / 10.0 ) for ii in range(5,26) ];
  dthresholds \
  ...
  with open( datadir+'/step21_try_a_minor_variation.csv', 'wt' ) as out:

    for p_threshold in pthresholds:

      print( '  {:1.2f}'.format( p_threshold ) );
      ...
      for d_threshold in dthresholds:
        total_covered_by_d[ d_threshold ] = set();
        pos_covered_by_d[ d_threshold ] = set();
      ...
      for d_threshold in dthresholds:
        ...
        for (i,ref_row) in enumerate( data_pos ):
          ...
          if p > p_threshold:
            total_covered_by_d[ d_threshold ] |= pos_in_vicinity;
            total_covered_by_d[ d_threshold ] |= neg_in_vicinity;
            pos_covered_by_d[ d_threshold ] |= pos_in_vicinity;
            # print( '  ', d_threshold, *stats );

        dmaxd = None;
        dmaxf = None;
        for d_threshold in dthresholds:
          if len( total_covered_by_d[d_threshold] ) <= 1:
            continue;
          recall = float( len(pos_covered_by_d[ d_threshold ]) ) / float( len(data_pos) );
          precision = float( len(pos_covered_by_d[ d_threshold ]) ) / float( len(total_covered_by_d[ d_threshold ]) );
          f1 = 2.0 * ( precision * recall ) / ( precision + recall );
          if ( dmaxf is None ) or ( f1 > dmaxf ):
            dmaxf = f1;
            dmaxd = d_threshold;

        if dmaxd is not None:
          total_covered |= total_covered_by_d[ dmaxd ];
          pos_covered |= pos_covered_by_d[ dmaxd ];
      ...
      if ( len(total_covered) > 1 ):
        ...
        f1 = 2.0 * ( precision * recall ) / ( precision + recall );
        print( '-> {:1.2f} {:1.4f} {:1.4f} {:1.4f}'.format( p_threshold, f1, precision, recall ) );
        print( '{:1.2f};{:1.4f};{:1.4f};{:1.4f}'.format( p_threshold, f1, precision, recall ), file=out );
